...
def test_patch_str():
    ...
    assert patch("42", [make_op(ADD, 0, "3"), make_op(REMOVE, 1)]) == "34"

    # Test -, single item deletion
    assert patch("3", [make_op(REMOVE, 0)]) == ""
    assert patch("42", [make_op(REMOVE, 0)]) == "2"
    assert patch("425", [make_op(REMOVE, 0)]) == "25"
    assert patch("425", [make_op(REMOVE, 1)]) == "45"
    assert patch("425", [make_op(REMOVE, 2)]) == "42"
    ...
    assert patch("world", [make_op(REMOVE, 0), make_op(ADD, 0, "W")]) == "World"
    ...
    assert patch("abcd", [make_op(REMOVERANGE, 1, 2)]) == "ad"
    assert patch("abcd", [make_op(REMOVERANGE, 2, 2)]) == "ab"
...
def test_patch_list():
    ...
    # Test -, single item deletion
    assert patch([3], [make_op(REMOVE, 0)]) == []
    assert patch([5, 6, 7], [make_op(REMOVE, 0)]) == [6, 7]
    assert patch([5, 6, 7], [make_op(REMOVE, 1)]) == [5, 7]
    assert patch([5, 6, 7], [make_op(REMOVE, 2)]) == [5, 6]
    assert patch([5, 6, 7], [make_op(REMOVE, 0), make_op(REMOVE, 2)]) == [6]
    ...
    assert patch(["hello", "world"], [make_op(PATCH, 0, [make_op(REPLACE, 0, "H")]),
                                      make_op(PATCH, 1, [make_op(REMOVE, 0), make_op(ADD, 0, "W")])]) == ["Hello", "World"]
    ...
    assert patch([5, 6, 7, 8], [make_op(REMOVERANGE, 1, 2)]) == [5, 8]
    assert patch([5, 6, 7, 8], [make_op(REMOVERANGE, 2, 2)]) == [5, 6]
...
def test_patch_dict():
    ...
    # Test -, single item deletion
    assert patch({"a": 1}, [make_op(REMOVE, "a")]) == {}
    assert patch({"a": 1, "b": 2}, [make_op(REMOVE, "a")]) == {"b": 2}
    ...
    # Test !, item patch
    subdiff = [make_op(PATCH, 0, [make_op(REPLACE, 0, "H")]), make_op(PATCH, 1, [make_op(REMOVE, 0), make_op(ADD, 0, "W")])]
