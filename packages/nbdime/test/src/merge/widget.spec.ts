...
import type { IMergeDecision } from '../../../src/merge/decisions';
...
jest.mock('@jupyterlab/codemirror', () => {
  return {
    ...
    CodeMirrorEditorFactory: jest.fn().mockImplementation(() => {
      return {
        newInlineEditor: jest
          .fn()
          .mockImplementation((options: CodeEditor.IOptions) => {
            return {
              model: {
              ...
              editor: {
                getValue: jest
                  .fn()
                  .mockImplementation(() => options.model.value.text),
                on: jest.fn(),
                operation: jest.fn(),
                state: {}
              }
  ...
  describe('widget', () => {
    ...
    describe('MetadataDiffWidget', () => {
      it('should create a widget for an unchanged model', () => {
        ...
        let model = new MetadataMergeModel(base, []);
