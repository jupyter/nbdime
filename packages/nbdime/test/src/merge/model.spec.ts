          ...
          common_path: ['cells', 0]
			...
			describe('cell level decision', () => {
				let decs: MergeDecision[] = [
					new MergeDecision(decPatchLvsDelR),
				];
				...
				it('should split sub-cell level decisions', () => {
					...
					expect(subdec.absolutePath).toEqual([
						'cells',
						0,
						'outputs',
					]);
		...
		describe('NotebookMergeModel', () => {
			...
			describe('decision splitting', () => {
				const diff: IDiffEntry[] = [
					opPatch(0, [
						opPatch('metadata', [opReplace('collapsed', true)]),
					]),
					opPatch(2, [
						opPatch('source', [opAddRange(1, ['    z += 2\n'])]),
					]),
				];
