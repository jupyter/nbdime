...
describe('patch', () => {
  describe('patchStringified', () => {
    ...
    it('should patch a nested patch', () => {
      ...
      console.log(
        value.remote.slice(value.additions[0].from, value.additions[0].to),
      );
      expect(value.additions).toEqual([{ from: f, to: t, source: undefined }]);
      t = f + 'valid'.length;
      expect(value.deletions).toEqual([{ from: f, to: t, source: undefined }]);
  ...
  describe('patch', () => {
    ...
    describe('patch sequence', () => {
      ...
      it('should fail to patch a sequence with a non-number key', () => {
        ...
        expect(() => {
          patch(base, diff);
        }).toThrow(/Invalid patch sequence op: Key is not a number: text/);

        // Not so obvious case (pure type error, which could be cast correctly)
        diff = [makeRemove('32')];
        expect(() => {
          patch(base, diff);
        }).toThrow(/Invalid patch sequence op: Key is not a number: 32/);
      });
      ...
      it('should fail to patch a sequence with an add on invalid key', () => {
        ...
        expect(() => {
          patch(base, diff);
        }).toThrow(/Invalid add range diff op: Key out of range: Infinity/);
      ...
      it('should fail to patch a sequence with a remove on invalid key', () => {
        ...
        expect(() => {
          patch(base, diff);
        }).toThrow(/Invalid remove range diff op: Key out of range: -1/);
        ...
        expect(() => {
          patch(base, diff);
        }).toThrow(/Invalid remove range diff op: Key out of range: Infinity/);

        diff = [makeRemoveRange(NaN, 1)];
        expect(() => {
          patch(base, diff);
        }).toThrow(/Invalid remove range diff op: Key out of range: NaN/);
      });
