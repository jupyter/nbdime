...
export function patchString(
	...
	for (let e of diff) {
		...
		if (e.op === 'addrange') {
			...
			additions.push(
				new DiffRangeRaw(remote.length, added.length, e.source),
			);
			remote += added;
			skip = 0;
	...
	each(helper.keys(), (key) => {
		...
		if (helper.isDiffKey(key)) {
			...
			if (valueIn(e.op, ['add', 'replace', 'remove'])) {
				...
				valr =
					keyString +
					valr.slice((level + 1) * JSON_INDENT.length) +
					postfix;
				let offset =
					remote.length +
					keyString.length -
			...
			// Entry unchanged
			let val =
				keyString + stringify(base[key], level + 1, false) + postfix;
			remote += val;
