...
export class DiffView {
	...
	protected modelInvalid(): boolean {
		return (
			this.model instanceof DecisionStringDiffModel && this.model.invalid
		);
	}
		...
		if (node && node.sources) {
			...
			if (gutter === GUTTER_PICKER_CLASS) {
				...
				for (let i = ss.length - 1; i >= 0; --i) {
					...
					if (
						this.type === 'merge' &&
						hasEntries(s.decision.customDiff)
					) {
			...
			if (j > -1) {
				...
				for (let k = 0; k < others.length; k++) {
					if (k === o) {
						...
						lines.push(
							getMatchingEditLineLC(chunk, others[k].lineChunks),
						);
					}
				}
				if (
					linesToAlign.length > j &&
					linesToAlign[j][0] === chunk.baseTo
				) {
...
export class MergeView extends Panel {
	constructor(options: IMergeViewEditorConfiguration) {
		...
		if (!main) {
			throw new Error(
				'Either remote or merged model needs to be specified!',
			);
		}
