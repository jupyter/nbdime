		...
		for (let dec of cellDecs) {
			if (
				...
				let mergeExecDiff = buildDiffs(base, [dec], 'merged') as
					| IDiffImmutableObjectEntry[]
					| null;
				let execDiff = hasEntries(mergeExecDiff)
					? mergeExecDiff[0]
					: null;
	...
	protected processDecisions(decisions: MergeDecision[]): void {
		...
		for (let md of decisions) {
			...
			if (md.absolutePath.length < 2 || md.absolutePath[0] !== 'cells') {
				...
				let splitDec = pushPatchDecision(
					md,
					md.absolutePath.slice(1, 2),
				);
     ...
     */
		...
		if (!hasEntries(md.localDiff)) {
			...
			if (this.base === null) {
				...
				// 2.
				this._remote = createDeletedCellDiffModel(
					this.base,
					this.mimetype,
				);
				this._merged = createDeletedCellDiffModel(
					this.base,
					this.mimetype,
				);
			...
			if (this.base === null) {
				...
				// 2.
				this._local = createDeletedCellDiffModel(
					this.base,
					this.mimetype,
				);
				this._merged = createDeletedCellDiffModel(
			...
			console.assert(
				md.localDiff.length === 1 && md.remoteDiff.length === 1,
			);
			// 3. or 4.
			if (md.localDiff[0].op === md.remoteDiff[0].op) {
				...
				if (this.base === null) {
					...
					this.deleteCell = valueIn(md.action, [
						'local',
						'remote',
						'either',
					]);
				...
				if (ops[0] === 'removerange') {
					...
					newDecisions = newDecisions.concat(
						this.splitPatch(
							md,
							null,
							md.remoteDiff[0] as IDiffPatchObject,
						),
					);
				} else {
					this._remote = createDeletedCellDiffModel(
					...
					newDecisions = newDecisions.concat(
						this.splitPatch(
							md,
							md.localDiff[0] as IDiffPatchObject,
							null,
						),
					);
				}
	...
	protected splitPatch(
		...
		if (local && remote) {
			...
			if (localPatch!.key !== remotePatch!.key) {
				throw new Error(
					'Different keys of patch ops given to `splitPatch`.',
				);
			}
		}
		...
		for (let key of keys) {
			if (this._whitelist && !valueIn(key, this._whitelist)) {
				throw new NotifyUserError(
					'Currently not able to handle decisions on variable "' +
						key +
						'"',
				);
			}
